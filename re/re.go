...
func RE(filepath, host string, port int, user, password, dbName, modelFileName, packageName, mysql8 string, version int) {
	...
	if filepath != "" {
		...
		myConfig.DBHost = host
		myConfig.DBPort = port
		myConfig.DBUser = user
		myConfig.DBPwd = password
		myConfig.DBName = dbName
		myConfig.ModelFileName = modelFileName
		myConfig.PackageName = packageName
		myConfig.Mysql8 = mysql8
